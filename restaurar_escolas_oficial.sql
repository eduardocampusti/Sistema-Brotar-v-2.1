...
-- RESTAURAÇÃO DE UNIDADES ESCOLARES - SISTEMA BROTAR
-- Projeto: indshiztdvjgvgnzigqd

-- 1. Limpa dados antigos (se houver) para evitar duplicidade
...
-- 3. Verifica se o perfil admin existe; se não, cria (ID é match com o email admin@brotar.com no Auth se já existir)
-- Nota: Se o usuário admin não conseguir logar, Eduardo precisará recriá-lo no Auth primeiro.
INSERT INTO public.profiles (full_name, username, role, is_active, email)
SELECT 'Eduardo Admin', 'admin', 'ADMIN', true, 'admin@brotar.com'
WHERE NOT EXISTS (SELECT 1 FROM public.profiles WHERE email = 'admin@brotar.com');
