import{j as e}from"./vendor-utils-BfcERQCZ.js";import{b as d,R as M,S as z,c as G,a6 as T,l as B,G as F,aA as q,i as U,V,ab as H,aY as Y,aZ as Z,aq as Q,a_ as W,au as X}from"./vendor-ui-xqE3C1mS.js";import{b as J,a as N}from"./index-CI37wG_3.js";import{G as K}from"./vendor-ai-BER3QIUg.js";import"./vendor-pdf-ddCP4G2p.js";import"./vendor-supabase-CbM-anGD.js";const ee=new K({apiKey:"AIzaSyCyp09yV8UF1pYLx3ztn1B4LFakUEMmXIU"}),te=`
...
<div style="${y}">
    ...
    `.trim()}},ie=["Declaração de Atendimento","Encaminhamento Geral","Relatório Resumido","Termo de Autorização de Uso de Imagem e Vídeo","Declaração Simples"],re=["Relatório Psicológico Técnico","Evolução Psicológica","Parecer Psicológico"],ne=["Relatório de Busca Ativa","Relatório Social de Visita Domiciliar","Ofício ao Conselho Tutelar","Plano de Acompanhamento Familiar"],_e=["Avaliação Psicopedagógica","Plano de Intervenção","Relatório de Evolução"],ue=({currentUser:r})=>{const{addToast:a}=J(),[x,g]=d.useState("generator"),[c,D]=d.useState([]),[n,y]=d.useState(""),[h,i]=d.useState(""),[m,f]=d.useState(""),[u,b]=d.useState(""),[t,p]=d.useState(""),[_,j]=d.useState(!1),[$,v]=d.useState([]);d.useEffect(()=>{(async()=>{const s=await N.getStudents();D(s)})()},[]);const[E,l]=d.useState(null),I=async()=>{if(!n||!h){l("Por favor, selecione um aluno e o tipo de documento desejado.");return}const s=(await N.getStudents()).find(A=>A.id===n);if(!s){l("Erro ao atualizar dados do aluno. Tente novamente.");return}const w=s;j(!0),l(null);const C=`BRT-${new Date().getFullYear()}-${Math.floor(Math.random()*9e4)+1e4}`;let S,R=!1;try{S=await oe.generateOfficialDocument(h,s,r.name,r.jobTitle||r.role,m)}catch(A){console.warn("IA indisponível, ativando fallback de templates:",A);const P=r.specialty||r.jobTitle||r.role;S=se.getFallbackDocument(h,w,r.name,P,m),R=!0}b(S),p(C);const k={id:crypto.randomUUID(),studentId:w.id,studentName:w.fullName,docType:h,code:C,content:S,createdAt:new Date().toISOString(),professionalName:r.name};try{if(await N.saveDocument(k),l(null),R&&a("Documento gerado com sucesso usando modelos institucionais (IA instável).","info"),x==="history"){const A=await N.getDocuments(n);v(A)}}catch(A){console.error("Erro ao salvar documento gerado:",A),l("O texto foi gerado, mas não foi possível salvar no histórico. Verifique conexão, permissões ou tente novamente."),a("Falha ao salvar o documento no servidor.","error")}finally{j(!1)}},O=async()=>{const o=window.open("","_blank","width=900,height=800");if(!o)return;const s=await N.getPapelTimbradoConfig();console.log("DEBUG: Config usada na impressão:",s);const w=`
      <html>
        <head>
