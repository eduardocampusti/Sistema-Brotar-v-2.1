import{j as e}from"./vendor-utils-DrLvZqkZ.js";import{b as N,o as _,p as W,O as V,S as ee,c as te,a9 as ae}from"./vendor-ui-9VhDV4ZH.js";import{u as oe,a as U}from"./index-Djgd218X.js";import{G as se}from"./geminiService-DTfoLb1U.js";import"./vendor-pdf-CpiGyxgF.js";import"./vendor-supabase-CbM-anGD.js";import"./vendor-ai-BER3QIUg.js";const X=new Date().getFullYear(),ie=Array.from({length:5},(s,m)=>X-m),ge=({currentUser:s})=>{const{addToast:m}=oe(),[n,g]=N.useState(X),[r,l]=N.useState(!1),[j,h]=N.useState(null),[u,c]=N.useState(""),[y,T]=N.useState(""),f=s.specialty||"Não informada",b=s.role==="EDUCATION_SECRETARY"||s.role==="ADMIN",E=async()=>{l(!0),h(null),c("");try{const d=`${n}-01-01`,z=`${n}-12-31`,$={fromDate:d,toDate:z};b||($.professionalId=s.id);const R=await U.getAppointments($);if(R.length===0){h(`Nenhum atendimento encontrado para ${s.name} no ano ${n}.`),l(!1);return}const a=R,i=a.length,A=a.filter(t=>["ATENDIDO","ENCERRADO","EM_ATENDIMENTO"].includes(t.status)).length,B=a.filter(t=>t.status==="FALTOU").length,D=a.filter(t=>t.status==="CANCELADO").length,k=a.filter(t=>t.status==="RETROATIVO").length,M=new Set(a.map(t=>t.studentId)).size,J=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"],I={};a.forEach(t=>{if(t.date){const o=parseInt(t.date.split("-")[1])-1;I[o]=(I[o]||0)+1}});const Y=Array.from(new Set(a.map(t=>t.studentId)));let P=[];try{P=await U.getStudents()}catch{}const S=P.filter(t=>Y.includes(t.id));if(S.length>0){const t=S[0];console.log("[TCM DEBUG] Campos escola do aluno:",{school:t.school,schoolName:t.schoolName,school_name:t.school_name,schoolInfo:t.schoolInfo})}const v={};a.forEach(t=>{var G,F;const o=S.find(K=>K.id===t.studentId),x=((G=o==null?void 0:o.school)==null?void 0:G.schoolName)||(o==null?void 0:o.schoolName)||(o==null?void 0:o.school_name)||((F=o==null?void 0:o.schoolInfo)==null?void 0:F.schoolName)||(typeof(o==null?void 0:o.school)=="string"?o.school:null)||"Escola não informada";v[x]||(v[x]={alunos:new Set,atendimentos:0}),v[x].alunos.add(t.studentId),v[x].atendimentos++});const H=[...a].filter(t=>["ATENDIDO","ENCERRADO"].includes(t.status)).sort((t,o)=>{var x;return(x=o.date)==null?void 0:x.localeCompare(t.date)}).slice(0,10),p={};a.forEach(t=>{const o=t.specialty||f;p[o]||(p[o]={total:0,atendidos:0,alunos:new Set}),p[o].total++,["ATENDIDO","ENCERRADO"].includes(t.status)&&p[o].atendidos++,p[o].alunos.add(t.studentId)});const O={};a.forEach(t=>{const o=t.unit||"Não informada";O[o]=(O[o]||0)+1});const L=b?Array.from(new Set(a.map(t=>t.professionalName))).filter(Boolean):[s.name],q=i>0?Math.round(A/i*100):0,Q=`
DADOS REAIS DO SISTEMA BROTAR — ANO ${n}
Profissional: ${s.name}
Especialidade: ${f}
Total de agendamentos: ${i}
Atendimentos realizados: ${A}
Faltas: ${B} | Cancelamentos: ${D} | Retroativos: ${k}
Alunos únicos atendidos: ${M}
Taxa de comparecimento: ${q}%
Profissionais: ${L.join(", ")}
Por especialidade: ${Object.entries(p).map(([t,o])=>`${t}: ${o.total} agend., ${o.atendidos} realizados, ${o.alunos.size} alunos`).join(" | ")}
Por unidade: ${Object.entries(O).map(([t,o])=>`${t}: ${o}`).join(" | ")}
      `.trim();let C;try{const t=`Você é redator oficial do Programa BROTAR de Brotas de Macaúbas/BA. Com base nos dados reais abaixo, complemente e enriqueça o relatório anual institucional para prestação de contas ao TCM/BA referente ao ano ${n}. Use linguagem técnica, formal e institucional. Para cada seção, escreva parágrafos completos e detalhados baseados nos dados fornecidos. NÃO deixe campos em branco — substitua todos os "XXXX" pelos valores reais. ${Q}`;C=await se.generateOfficialDocument("Relatório Anual TCM",{fullName:"Rede Municipal",school:{schoolName:"Brotas de Macaúbas"}},s.name,"Secretária de Educação",t)}catch{C=de(n,f,s.name,L,A,M,B,D,k,i,q,p,O,I,J,v,H)}const Z=`TCM-${n}-${Math.floor(Math.random()*9e4)+1e4}`;T(Z),c(C),m("Relatório anual gerado com sucesso!","success")}catch{h("Erro ao gerar relatório. Verifique a conexão e tente novamente.")}finally{l(!1)}},w=()=>{const d=window.open("","_blank","width=1000,height=900");d&&(d.document.write(`<!DOCTYPE html><html><head><title>Relatório Anual TCM ${n}</title>
    <style>
      @page { size: A4; margin: 0; }
      body { margin: 0; padding: 0; font-family: 'Times New Roman', serif; }
      @media print { .no-print { display: none; } }
    </style></head><body>${u}</body></html>`),d.document.close(),setTimeout(()=>{d.print()},800))};return e.jsxs("div",{className:"max-w-6xl mx-auto space-y-6 pb-12",children:[e.jsxs("div",{className:"rounded-3xl p-6 text-white",style:{background:"linear-gradient(135deg, #8B1A3A, #6B1230)"},children:[e.jsxs("div",{className:"flex items-center gap-3 mb-2",children:[e.jsx("div",{className:"p-2 rounded-xl",style:{background:"rgba(255,255,255,0.15)"},children:e.jsx(_,{size:24})}),e.jsxs("div",{children:[e.jsx("h1",{className:"text-2xl font-black tracking-tight",children:"Relatório Anual — TCM"}),e.jsx("p",{className:"text-sm",style:{color:"rgba(255,255,255,0.70)"},children:"Tribunal de Contas dos Municípios da Bahia"})]})]}),e.jsx("p",{className:"text-xs mt-3",style:{color:"rgba(255,255,255,0.60)"},children:"Gera automaticamente o relatório institucional anual seguindo o modelo oficial do Programa BROTAR, com dados reais do sistema preenchidos pela IA."})]}),e.jsxs("div",{className:"grid grid-cols-1 lg:grid-cols-3 gap-6",children:[e.jsx("div",{className:"lg:col-span-1",children:e.jsxs("div",{className:"bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-5 sticky top-6",children:[e.jsx("h2",{className:"text-sm font-black text-slate-700 uppercase tracking-wider",children:"Configurar Relatório"}),e.jsxs("div",{children:[e.jsx("label",{className:"block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2",children:"Ano de Referência"}),e.jsxs("div",{className:"relative",children:[e.jsx(W,{className:"absolute left-3 top-1/2 -translate-y-1/2 text-slate-400",size:16}),e.jsx("select",{className:"w-full pl-9 pr-4 py-3 rounded-xl border border-slate-200 bg-slate-50/30 text-slate-700 font-semibold appearance-none text-sm",value:n,onChange:d=>g(Number(d.target.value)),children:ie.map(d=>e.jsx("option",{value:d,children:d},d))})]})]}),e.jsxs("div",{children:[e.jsx("label",{className:"block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2",children:"Profissional"}),e.jsxs("div",{className:"bg-slate-50 rounded-xl p-3 border border-slate-100",children:[e.jsx("p",{className:"text-sm font-bold text-slate-800",children:s.name}),e.jsx("p",{className:"text-xs text-slate-500 mt-0.5",children:f}),e.jsxs("div",{className:"mt-2 inline-flex items-center gap-1.5 bg-emerald-50 border border-emerald-100 rounded-lg px-2 py-1",children:[e.jsx("div",{className:"w-1.5 h-1.5 rounded-full bg-emerald-400"}),e.jsx("span",{className:"text-[10px] font-semibold text-emerald-700",children:"Relatório individual"})]})]})]}),e.jsxs("button",{onClick:E,disabled:r,className:"w-full py-4 text-white rounded-xl font-bold hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95",style:{background:"linear-gradient(135deg, #8B1A3A, #6B1230)"},children:[r?e.jsx(V,{size:18,className:"animate-spin"}):e.jsx(ee,{size:18}),r?"Gerando com IA...":"Gerar Relatório TCM"]}),j&&e.jsxs("div",{className:"p-3 bg-red-50 border border-red-200 rounded-xl flex gap-2 items-start",children:[e.jsx(te,{size:16,className:"text-red-500 shrink-0 mt-0.5"}),e.jsx("p",{className:"text-red-600 text-xs",children:j})]}),e.jsx("div",{className:"border-t border-slate-100 pt-4",children:e.jsx("p",{className:"text-[10px] text-slate-400 font-medium",children:"📄 Modelo baseado no PDF oficial do Programa BROTAR com 13 seções, tabela de indicadores e página de assinaturas."})})]})}),e.jsx("div",{className:"lg:col-span-2",children:u?e.jsxs("div",{className:"bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden",children:[e.jsxs("div",{className:"flex items-center justify-between px-5 py-3 bg-slate-50 border-b border-slate-100",children:[e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsxs("span",{className:"text-xs font-black text-slate-500 bg-slate-200 px-2 py-1 rounded-lg",children:["#",y]}),e.jsxs("span",{className:"text-xs text-slate-500 font-medium",children:["Relatório Anual ",n]})]}),e.jsxs("button",{onClick:w,className:"flex items-center gap-2 px-4 py-2 text-white rounded-xl text-xs font-bold transition-all",style:{background:"#8B1A3A"},children:[e.jsx(ae,{size:14})," Imprimir PDF"]})]}),e.jsx("div",{className:"overflow-y-auto max-h-[700px]",dangerouslySetInnerHTML:{__html:u}})]}):e.jsx("div",{className:"bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center min-h-[400px] text-center p-8",children:r?e.jsxs(e.Fragment,{children:[e.jsx(V,{size:48,className:"animate-spin mb-4",style:{color:"#8B1A3A"}}),e.jsx("p",{className:"font-bold text-slate-700",children:"Coletando dados e gerando relatório..."}),e.jsx("p",{className:"text-slate-400 text-sm mt-2",children:"A IA está preenchendo todas as 13 seções"})]}):e.jsxs(e.Fragment,{children:[e.jsx("div",{className:"w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center mb-4 border border-slate-100",children:e.jsx(_,{size:32,className:"text-slate-300"})}),e.jsx("p",{className:"font-bold text-slate-500",children:"Configure e gere o relatório"}),e.jsx("p",{className:"text-slate-400 text-sm mt-1",children:"O modelo seguirá o padrão oficial do PDF do BROTAR"})]})})})]})]})};function de(s,m,n,g,r,l,j,h,u,c,y,T,f,b,E,w,d){const z=new Date().toLocaleDateString("pt-BR",{day:"2-digit",month:"long",year:"numeric"}),$=m==="TODAS"?"Todas as especialidades":m,R=Object.entries(T).map(([a,i])=>`
    <tr>
...
<div style="font-family:'Times New Roman',serif;color:#000;line-height:1.6;max-width:800px;margin:0 auto">
  ...
  <div style="padding:40px;box-sizing:border-box">
    ...
    <ul style="margin:8px 0 8px 20px">
      ${g.length>0?g.map(a=>`<li>${a}</li>`).join(""):"<li>Equipe multiprofissional conforme quadro funcional vigente</li>"}
    </ul>
    ...
    <table style="width:100%;border-collapse:collapse;margin:8px 0;font-size:10pt">
      ...
      ${Array.from({length:12},(a,i)=>i).filter(a=>b[a]>0).map((a,i)=>`
      <tr style="${i%2===0?"background:#f5f5f5":""}">
        <td style="border:1px solid #ccc;padding:6px 10px">${E[a]}</td>
        <td style="border:1px solid #ccc;padding:6px 10px;text-align:center"><strong>${b[a]}</strong></td>
        <td style="border:1px solid #ccc;padding:6px 10px;text-align:center">${c>0?Math.round(b[a]/c*100):0}%</td>
      </tr>`).join("")}
    </table>
    ...
    <table style="width:100%;border-collapse:collapse;margin:8px 0;font-size:10pt">
      ...
      ${Object.entries(w).sort((a,i)=>i[1].atendimentos-a[1].atendimentos).map(([a,i],A)=>`
  ...
  `.trim()}export{ge as RelatorioAnualTCM};
