import{j as e}from"./vendor-utils-DrLvZqkZ.js";import{b as N,o as G,p as K,O as F,S as W,c as ee,a9 as te}from"./vendor-ui-9VhDV4ZH.js";import{u as ae,a as V}from"./index-CUaXRSoz.js";import{G as oe}from"./geminiService-DTfoLb1U.js";import"./vendor-pdf-CpiGyxgF.js";import"./vendor-supabase-CbM-anGD.js";import"./vendor-ai-BER3QIUg.js";const _=new Date().getFullYear(),se=Array.from({length:5},(s,m)=>_-m),me=({currentUser:s})=>{const{addToast:m}=ae(),[n,g]=N.useState(_),[r,l]=N.useState(!1),[j,h]=N.useState(null),[u,c]=N.useState(""),[y,T]=N.useState(""),f=s.specialty||"Não informada",b=s.role==="EDUCATION_SECRETARY"||s.role==="ADMIN",E=async()=>{l(!0),h(null),c("");try{const d=`${n}-01-01`,z=`${n}-12-31`,$={fromDate:d,toDate:z};b||($.professionalId=s.id);const R=await V.getAppointments($);if(R.length===0){h(`Nenhum atendimento encontrado para ${s.name} no ano ${n}.`),l(!1);return}const a=R,i=a.length,A=a.filter(t=>["ATENDIDO","ENCERRADO","EM_ATENDIMENTO"].includes(t.status)).length,C=a.filter(t=>t.status==="FALTOU").length,B=a.filter(t=>t.status==="CANCELADO").length,D=a.filter(t=>t.status==="RETROATIVO").length,k=new Set(a.map(t=>t.studentId)).size,U=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"],I={};a.forEach(t=>{if(t.date){const o=parseInt(t.date.split("-")[1])-1;I[o]=(I[o]||0)+1}});const X=Array.from(new Set(a.map(t=>t.studentId)));let M=[];try{M=await V.getStudents()}catch{}const J=M.filter(t=>X.includes(t.id)),v={};a.forEach(t=>{var q;const o=J.find(Z=>Z.id===t.studentId),x=((q=o==null?void 0:o.school)==null?void 0:q.schoolName)||(o==null?void 0:o.schoolName)||"Escola não informada";v[x]||(v[x]={alunos:new Set,atendimentos:0}),v[x].alunos.add(t.studentId),v[x].atendimentos++});const Y=[...a].filter(t=>["ATENDIDO","ENCERRADO"].includes(t.status)).sort((t,o)=>{var x;return(x=o.date)==null?void 0:x.localeCompare(t.date)}).slice(0,10),p={};a.forEach(t=>{const o=t.specialty||f;p[o]||(p[o]={total:0,atendidos:0,alunos:new Set}),p[o].total++,["ATENDIDO","ENCERRADO"].includes(t.status)&&p[o].atendidos++,p[o].alunos.add(t.studentId)});const O={};a.forEach(t=>{const o=t.unit||"Não informada";O[o]=(O[o]||0)+1});const P=b?Array.from(new Set(a.map(t=>t.professionalName))).filter(Boolean):[s.name],L=i>0?Math.round(A/i*100):0,H=`
DADOS REAIS DO SISTEMA BROTAR — ANO ${n}
Profissional: ${s.name}
Especialidade: ${f}
Total de agendamentos: ${i}
Atendimentos realizados: ${A}
Faltas: ${C} | Cancelamentos: ${B} | Retroativos: ${D}
Alunos únicos atendidos: ${k}
...
<div style="font-family:'Times New Roman',serif;color:#000;line-height:1.6;max-width:800px;margin:0 auto">
  ...
  `.trim()}export{me as RelatorioAnualTCM};
