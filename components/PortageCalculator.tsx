...
export const PortageCalculator: React.FC<PortageCalculatorProps> = ({ student, onSave, currentUser }) => {
    ...
    const [scores, setScores] = useState<Record<string, number[]>>({
        socializacao: [0,0,0,0,0,0], linguagem: [0,0,0,0,0,0],
        cognicao: [0,0,0,0,0,0], autocuidados: [0,0,0,0,0,0], motor: [0,0,0,0,0,0]
    });
    ...
    const handleSave = async () => {
        ...
        try {
            const assessment: PortageAssessment = {
                id: crypto.randomUUID(), studentId: student.id,
                date: new Date().toISOString(), professionalName: currentUser.name,
                studentAgeYears: studentAgeInfo.years, studentAgeMonths: studentAgeInfo.months,
                scores: scores as any,
                results: { socializacao: result.domainResults.socializacao, linguagem: result.domainResults.linguagem, cognicao: result.domainResults.cognicao, autocuidados: result.domainResults.autocuidados, motor: result.domainResults.motor, general: result.generalResult },
        ...
        } catch (err) { console.error(err); addToast("Erro ao salvar avaliação.", "error"); }
    ...
    const barData = DOMAINS.map(d => ({ name: d.label, 'Idade Real': Math.min(studentAgeInfo.years + studentAgeInfo.months/12, 6), 'Idade Desenv.': result?.domainResults[d.key] || 0 }));
    ...
    return (
        <div className="space-y-4 animate-fadeIn">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    <h2 className="text-base font-bold text-slate-800 flex items-center gap-2"><Calculator size={16} className="text-[#10B981]" /> Calculadora IPO — Portage</h2>
                    <div className="flex items-center gap-2 mt-1.5">
                        <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1 rounded-xl bg-[#E6F1FB] text-[#185FA5] border border-[#85B7EB] font-bold"><Calculator size={11} /> {studentAgeInfo.years} anos e {studentAgeInfo.months} meses</span>
            ...
            <div className="flex items-center gap-2 text-xs px-3 py-2 rounded-xl bg-[#E6F1FB] text-[#185FA5] border border-[#85B7EB] font-bold">
                <Info size={13} /> {inputMode==='CHECKLIST' ? 'Marque cada item. "Com ajuda" vale 0,5 ponto. Score calculado automaticamente.' : 'Faixa etária atual destacada. Preencha as faixas abaixo da idade cronológica.'}
            </div>
                                ...
                                <button onClick={() => setExpandedRanges(prev => ({...prev,[key]:!isExpanded}))}
                                    ...
                                    <div className="flex items-center gap-3">
                                        <span className="text-xs font-bold" style={{color:isCurrent?style.text:'var(--color-text-primary)'}}>
                                            {isCurrent&&'↳ '}{rangeLabel}
                                            {isCurrent&&<span className="ml-2 text-[9px] px-1.5 py-0.5 rounded" style={{background:'#8B1A3A',color:'#fff'}}>atual</span>}
                                        </span>
                    ...
                    <table className="min-w-[500px] w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50">
                                ...
                                {DOMAINS.map(d => (<th key={d.key} className="px-3 py-3 text-center text-[10px] font-black uppercase tracking-widest" style={{color:DOMAIN_STYLES[d.key].text}}>{d.label}</th>))}
                            </tr>
                        </thead>
                        <tbody>
                            {AGE_RANGES.map((rangeLabel, rIndex) => {
